<template>
  <div class="page">
    ...
    <div class="contenido">
      ...
      <!-- VISTA MAPA -->
      <div v-if="vista === 'mapa'" class="mapa-section">
        <div v-if="cargando" class="estado">
          <div class="spinner"></div>
          <p>Cargando mapa...</p>
        </div>
        <div v-else>
          ...
          <div v-if="tecnicos.filter(t => t.latitud && t.longitud).length === 0" class="mapa-aviso">
            ℹ️ Los técnicos aún no han actualizado su ubicación. Pídeles que ingresen a "Mi perfil" y actualicen su
            ubicación.
          </div>
        </div>
      </div>
...
function limpiarUbicacion() {
  latitud.value = null; longitud.value = null
  usandoUbicacion.value = false; buscar()
}
